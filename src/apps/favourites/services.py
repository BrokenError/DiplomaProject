...
class FavouriteService(ServiceBase):
    ...
    async def get_fragment(self, query: Select, limit: Optional[int], offset: int) -> Tuple[List, int]:
        ...
        for favourite in favourites:
            favourite.product.photos = [photo for photo in favourite.product.photos if not photo.is_banner]

        return favourites, (await self.manager.execute(query_count)).scalar()
